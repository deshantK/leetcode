def search(word, index, row, col, grid, dic):
  if index == len(word):
    return True
  ...
  check = search(word, index+1,row+1, col, grid, dic)
  check = check or search(word, index+1, row-1, col, grid, dic)
  check = check or search(word, index+1, row, col+1, grid, dic)
  check = check or search(word, index+1, row, col-1, grid, dic)
  grid[row][col] = word[index]
  return check

def s_dp(word, grid, dic):
  index = len(word)
  check = False
  while index > 0:
    s_w = word[0:index]
    if s_w in dic:
      row, col = dic[s_w]
      check = search(word, index-1, row, col, grid, dic)
      if check:
        return True
    index = index-1
  return False
      
def find(grid, word, dic):
  for i in range(len(grid)):
   for j in range(len(grid[0])):
      if search(word, 0, i, j, grid, dic):
        return True
  return False 


grid = [["b","a","b","a","b","a","b","a","b","a"],["a","b","a","b","a","b","a","b","a","b"],["b","a","b","a","b","a","b","a","b","a"],["a","b","a","b","a","b","a","b","a","b"],["b","a","b","a","b","a","b","a","b","a"],["a","b","a","b","a","b","a","b","a","b"],["b","a","b","a","b","a","b","a","b","a"],["a","b","a","b","a","b","a","b","a","b"],["b","a","b","a","b","a","b","a","b","a"],["a","b","a","b","a","b","a","b","a","b"]]
...
ans = []
for word in words:
  check = s_dp(word, grid, dic)
  if check:
    ans.append(word)
  else:
    check = find(grid, word, dic)
    if check:
      ans.append(word)
print(ans)
print (dic)
